...
class AICodeAgent:
        ...
        try:
            response = self.groq_client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are an AI assistant specialized in generating Python code.",
                    },
                    {
                        "role": "user",
        ...
        try:
            response = self.groq_client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are an AI assistant specialized in analyzing Python code.",
                    },
                    {
                        "role": "user",
    ...
    def display_learned_info(self) -> None:
        ...
        for context, info_list in self.learned_info.items():
            ...
            for item in info_list:
                ...
                console.print(
                    Syntax(item["code"], "python", theme="monokai", line_numbers=True)
                )
                console.print("---")
    ...
    def save_learned_info(self, filename: str = "learned_info.json") -> None:
        try:
            ...
            console.print(
                f"[bold red]IOError while saving learned information: {str(e)}[/bold red]"
            )
        except Exception as e:
            console.print(
                f"[bold red]Error saving learned information: {str(e)}[/bold red]"
            )
    ...
    def load_learned_info(self, filename: str = "learned_info.json") -> None:
        try:
            ...
            console.print(
                f"[bold red]Error decoding JSON from {filename}: {str(e)}[/bold red]"
            )
        except Exception as e:
            console.print(
                f"[bold red]Error loading learned information: {str(e)}[/bold red]"
            )
...
class ScriptDebugger:
    ...
    def debug_and_improve(self) -> None:
        console.print(
            Panel(f"Debugging script: {self.script_path}", style="bold green")
        )
    ...
    def update_script_file(self, improved_code: str) -> None:
        try:
            ...
            console.print(
                f"[green]Script file updated with improved code: {self.script_path}[/green]"
            )
        except IOError as e:
            console.print(
                f"[bold red]IOError while updating script file: {str(e)}[/bold red]"
            )
        except Exception as e:
...
def debug_script(script_path: str) -> None:
    ...
    while True:
        ...
        if (
            not Prompt.ask(
                "Do you want to debug this script again?", choices=["y", "n"]
            ).lower()
            == "y"
        ):
            break
...
if __name__ == "__main__":
    script_path = Prompt.ask(
        "Enter the path to the script you want to debug",
        default=os.path.join(os.getcwd(), "script_to_debug.py"),
    )
